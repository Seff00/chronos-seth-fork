...
def plot_results(weekly_df, weekly_prices_train, forecast, actual_prices, test_dates):
    ...
    ax.plot(test_dates, actual_prices, label='Actual (Test Period)',
            color='darkblue', linewidth=2.5, marker='D', markersize=8)
    ...
    ax.plot(test_dates, forecast[median_idx, :], label=f'Median Forecast ({direction_label})',
            color=forecast_color, linewidth=2.5, marker='s', markersize=7, linestyle='--')
    ...
    # Save plot
    output_path = 'experiments_zeroshot/univariate/forecast_weekly_plot.png'
